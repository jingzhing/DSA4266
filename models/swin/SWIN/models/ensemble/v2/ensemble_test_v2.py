...
from ensemble_data_v2 import build_dataloaders
from ensemble_metrics_v2 import evaluate_predictions
from ensemble_model_utils_v2 import build_ensemble_model
...
def main():
    ...
    log_section("ENSEMBLE V2 TESTING")
    ...
    print(
        f"Ensemble Acc: {ensemble_eval['acc']:.4f} | Ensemble Balanced Acc: {ensemble_eval['balanced_acc']:.4f}" +
        (f" | Ensemble AUC: {ensemble_eval['auc']:.4f}" if ensemble_eval["auc"] is not None else "")
    )
