...
from config import EFFICIENTNET_CONFIG, ENSEMBLE_CONFIG, SWIN_CONFIG
...
def run_model_on_loader(model, loader, device):
    ...
    return np.concatenate(all_logits), np.concatenate(all_y).astype(int), all_paths
...
def _extract_split_signature(bundle, split_name):
    ds = bundle[f"{split_name}_ds"]
    if hasattr(ds, "paths"):
        return list(ds.paths)
    if hasattr(ds, "indices") and hasattr(ds, "samples"):
        return [ds.samples[i][0] for i in ds.indices]
    if hasattr(ds, "samples"):
        return [s[0] for s in ds.samples]
    raise RuntimeError(f"Unable to build split signature for {split_name}")
...
def main():
    ...
    val_sig_swin = _extract_split_signature(swin_bundle, "val")
    val_sig_eff = _extract_split_signature(eff_bundle, "val")
    test_sig_swin = _extract_split_signature(swin_bundle, "test")
    test_sig_eff = _extract_split_signature(eff_bundle, "test")

    if val_sig_swin != val_sig_eff:
        raise RuntimeError(
            "Validation splits do not match. Use the same val_split_path and matching data ingestion across solo models."
        )
    if test_sig_swin != test_sig_eff:
        raise RuntimeError("Test file order does not match between solo models")
    ...
    log_section("VALIDATION FOR ENSEMBLE SEARCH")
    swin_val_logits, y_val_orig, _ = run_model_on_loader(swin_model, swin_bundle["val_loader"], device)
    eff_val_logits, y_val_orig_eff, _ = run_model_on_loader(eff_model, eff_bundle["val_loader"], device)
    if not np.array_equal(y_val_orig, y_val_orig_eff):
        raise RuntimeError("Validation labels do not match between models")
    ...
    swin_test_logits, y_test_orig, test_paths = run_model_on_loader(swin_model, swin_bundle["test_loader"], device)
    eff_test_logits, y_test_orig_eff, _ = run_model_on_loader(eff_model, eff_bundle["test_loader"], device)
    ...
    print("ENSEMBLE test @ frozen validation threshold:", best["threshold"])
