...
CHECKPOINT_DIR = Path("models/swin/balanced_recipe_run")
CHECKPOINT_NAME = "best_swin_t_balanced_recipe.pt"
...
class MixCollator:
    def __init__(self, mixup_alpha=0.1, cutmix_alpha=0.0, mix_prob=0.25):
    ...
    def __call__(self, batch):
        ...
        do_mix = np.random.rand() < self.mix_prob and self.mixup_alpha > 0
        if not do_mix:
            return {
                "images": imgs,
                "targets_a": labels,
        ...
        if self.cutmix_alpha > 0 and np.random.rand() < 0.5:
            ...
            x1, y1, x2, y2 = self.rand_bbox(imgs.size(), lam)
            imgs[:, :, y1:y2, x1:x2] = imgs[perm, :, y1:y2, x1:x2]
            lam_adjusted = 1.0 - ((x2 - x1) * (y2 - y1) / (imgs.size(-1) * imgs.size(-2)))
            return {
                "images": imgs,
                "targets_a": labels,
                "targets_b": labels_b,
                "lam": float(lam_adjusted),
                "mixed": True,
                "mix_type": "cutmix",
                "paths": paths,
            }
...
def make_loader(paths, labels, transform, shuffle, loader_name, train=False):
    ...
    if train:
        loader_kwargs["collate_fn"] = MixCollator(
...
def build_optimizer(model):
    ...
    param_groups = []
    if body_params:
        param_groups.append({"params": body_params, "lr": BASE_LR, "name": "body"})
    if head_params:
...
def sync_optimizer_with_model(optimizer, model):
    model_params = {id(p): p for p in model.parameters() if p.requires_grad}
    for group in optimizer.param_groups:
        kept = [p for p in group["params"] if id(p) in model_params]
        nameset = {id(p) for p in kept}
        missing = [p for p in model_params.values() if id(p) not in nameset and ((group.get("name") == "head" and p in list(model.head.parameters())) or (group.get("name") == "body" and p not in list(model.head.parameters())))]
        group["params"] = kept + missing
    ...
    with context:
        ...
        for batch_idx, batch in enumerate(pbar, start=1):
            if train:
                ...
                targets_a = batch["targets_a"].to(DEVICE, non_blocking=True)
                targets_b = batch["targets_b"].to(DEVICE, non_blocking=True)
                lam = batch["lam"]
                mixed = batch["mixed"]
                paths = batch["paths"]
            else:
            ...
            total_correct += (preds == hard_labels.long()).sum().item()
            total_seen += hard_labels.size(0)
            total_loss += loss.item() * hard_labels.size(0)

            all_probs.extend(probs.detach().cpu().numpy())
            all_true.extend(hard_labels.detach().cpu().numpy())
...
def main():
    ...
    log("=" * 80)
    log("SWIN-T BALANCED TRAINING RUN")
    log("=" * 80)
    log(f"Device               : {DEVICE}")
    log(f"AMP                  : {USE_AMP}")
    ...
    log(f"Prefetch factor      : {PREFETCH_FACTOR if USE_PREFETCH else 'n/a'}")
    ...
    log(f"CutMix alpha         : {CUTMIX_ALPHA}")
    log(f"Mix probability      : {MIX_PROB}")
    log(f"Use pos weight       : {USE_POS_WEIGHT}")
    ...
    train_transform = transforms.Compose([
        ...
        transforms.RandomErasing(p=0.05, scale=(0.02, 0.06), ratio=(0.5, 2.0)),
    ])
    ...
    train_loader = make_loader(TR_PATHS, TR_LABELS, train_transform, True, "train", train=True)
    val_loader = make_loader(VA_PATHS, VA_LABELS, eval_transform, False, "val", train=False)
    test_loader = make_loader(test_paths, test_labels, eval_transform, False, "test", train=False)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if optimizer is None or trainable_mode != prev_mode:
            optimizer, scheduler = build_optimizer(model)
            if ema is not None:
                ema = ModelEMA(model, decay=EMA_DECAY, device=DEVICE)
            prev_mode = trainable_mode
