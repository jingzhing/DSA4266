...
def main():
    ...
    if list(swin_bundle["base_val_tune_ds"].indices) != list(eff_bundle["base_val_tune_ds"].indices):
        raise RuntimeError("Val-tune splits do not match. Use the same shared split file.")
    ...
    log_section("VAL-TUNE FOR ENSEMBLE SEARCH")
    ...
    if not np.array_equal(y_val_orig, y_val_orig_eff):
        raise RuntimeError("Val-tune labels do not match between models")
    ...
    print("ENSEMBLE test @ frozen val-tune threshold:", best["threshold"])
