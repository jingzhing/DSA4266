...
def build_epoch_training_set(base_paths, base_labels, hard_real_examples, extra_copies=1):
...
def make_loader(paths, labels, transform, shuffle, loader_name, use_face_region=False):
    ...
    log(
        ...
        f"shuffle={shuffle} face_region={use_face_region}"
    )
    return loader
...
def main():
    ...
    test_loader = make_loader(test_paths, test_labels, eval_transform, False, "test", use_face_region=FACE_REGION_ENABLED)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_paths_epoch, train_labels_epoch = build_epoch_training_set(
            TR_PATHS,
            TR_LABELS,
            current_hard_real_examples,
        ...
        if current_hard_real_examples:
            log(f"[HARD MINING] duplicated hard real samples this epoch: {len(current_hard_real_examples)}")
