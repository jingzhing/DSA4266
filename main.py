...
def main() -> int:
    ...
    if not args.skip_prepare:
        prepare_cmd = [python_exe, "-m", "pipeline.cli", "prepare", "--config", str(setup_cfg)]
        if not args.no_force_prepare:
            prepare_cmd.append("--force")
        if args.with_video:
            prepare_cmd.append("--with-video")
        for url in args.video_url:
            prepare_cmd.extend(["--video-url", url])
